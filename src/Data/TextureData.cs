...
internal sealed unsafe class TextureData(string? filePath = null) : SharedData(filePath is { Length: > 4 } ? "tex_" + filePath : "tex_" + NewGuid()) {
    ...
    protected override void BuildRaylib() {
        ...
        if (EncodedBytes is { Length: > 0 }) {
            ...
            var image = LoadImageFromMemory(extension, EncodedBytes);
            
            RaylibTexture = LoadTextureFromImage(image);
            UnloadImage(image);
        ...
        RaylibTexture = LoadTexture(FilePath);
    }

    protected override void UnloadRaylib() {
        
        if (!RaylibTexture.HasValue) return;
        
        UnloadTexture(RaylibTexture.Value);
        RaylibTexture = null;
    }
    ...
    protected override void BuildOptix() {
        ...
        if (!TryLoadImageForOptix(out var image)) return;

        try {
            
            ImageFormat(ref image, PixelFormat.UncompressedR8G8B8A8);
            ...
            finally { UnloadImageColors(colors); }
        }
    ...
    public void EnsureOptixPixels() {
        ...
        if (OptixPixels is { Length: > 0 } && OptixWidth > 0 && OptixHeight > 0) return;
    ...
    private bool TryLoadImageForOptix(out Image image) {
        ...
        if (EncodedBytes is { Length: > 0 }) {
            ...
            image = LoadImageFromMemory(extension, EncodedBytes);
        ...
        if (RaylibTexture.HasValue) {
            image = LoadImageFromTexture(RaylibTexture.Value);
            return image.Data != null;
        }
