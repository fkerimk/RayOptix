...
namespace {
    ...
    std::string DescribeLastWindowsError(const char *libraryName) {
        ...
        stream << libraryName << " could not be loaded (Win32 error " << static_cast<unsigned long>(errorCode) << ": "
                << message << ")";
        return stream.str();
    }
    ...
    void CheckOptix(OptixResult result, const char *call) {
        if (result != OPTIX_SUCCESS) {
            std::ostringstream stream;
            stream << call << " failed with OptiX code " << result;
            throw OptixError(stream.str());
        }
    }

    void CheckNvrtc(nvrtcResult result, const char *call) {
    ...
    void IgnoreCudaDriver(CUresult result) {
        (void) result;
    }
    ...
    public:
        ...
        void Resize(int renderWidth, int renderHeight, int outputWidth, int outputHeight) {
            ...
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&denoisedBeautyBuffer_), beautySize),
                      "cudaMalloc(denoisedBeautyBuffer)");
            ...
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&accumulationBuffer_), accumulationSize),
                      "cudaMalloc(accumulationBuffer)");
            CheckCuda(cudaMemset(reinterpret_cast<void *>(accumulationBuffer_), 0, accumulationSize),
                      "cudaMemset(accumulationBuffer)");
    ...
    private:
...
#endif
            CheckCudaDriver(cuStreamCreate(&stream_, CU_STREAM_DEFAULT), "cuStreamCreate");
#if defined(_WIN32)
        ...
        void SetupDenoiser() {
            ...
            EnsureBuffer(denoiserScratchBuffer_, denoiserScratchCapacity_,
        ...
        [[nodiscard]] CUdeviceptr DenoiseBeauty() const {
        ...
        void PresentToOutputTexture(CUdeviceptr hdrSource, const NativeRenderSettings &settings) {
            ...
            CheckCuda(cudaGraphicsSubResourceGetMappedArray(&outputArray, textureInteropResource_, 0, 0),
            ...
            auto sourceWidth = static_cast<unsigned int>(width_);
            auto sourceHeight = static_cast<unsigned int>(height_);
            auto outputWidth = static_cast<unsigned int>(outputWidth_);
            auto outputHeight = static_cast<unsigned int>(outputHeight_);
            unsigned long long surfaceHandle = surfaceObject;
        ...
        void RecreateGuideBuffers() {
            ...
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&worldPositionBuffer_), worldPositionSize),
                      "cudaMalloc(worldPositionBuffer)");
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&normalGuideBuffer_), normalSize),
                      "cudaMalloc(normalGuideBuffer)");
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&roughnessGuideBuffer_), roughnessSize),
                      "cudaMalloc(roughnessGuideBuffer)");
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&diffuseAlbedoGuideBuffer_), albedoSize),
                      "cudaMalloc(diffuseAlbedoGuideBuffer)");
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&specularAlbedoGuideBuffer_), albedoSize),
                      "cudaMalloc(specularAlbedoGuideBuffer)");
        }
        ...
        void CreatePipeline() {
            ...
            logSize = sizeof(log);
            CheckOptix(optixProgramGroupCreate(context_, &raygenDesc, 1, &programGroupOptions, log, &logSize,
                                               &raygenProgramGroup_), "optixProgramGroupCreate(raygen)");
            ...
            logSize = sizeof(log);
            CheckOptix(optixProgramGroupCreate(context_, &missDesc, 1, &programGroupOptions, log, &logSize,
                                               &missProgramGroup_), "optixProgramGroupCreate(miss)");
            ...
            logSize = sizeof(log);
            CheckOptix(optixProgramGroupCreate(context_, &hitDesc, 1, &programGroupOptions, log, &logSize,
                                               &hitgroupProgramGroup_), "optixProgramGroupCreate(hitgroup)");
            ...
            CheckOptix(optixUtilAccumulateStackSizes(raygenProgramGroup_, &stackSizes, pipeline_),
                       "optixUtilAccumulateStackSizes(raygen)");
            CheckOptix(optixUtilAccumulateStackSizes(missProgramGroup_, &stackSizes, pipeline_),
                       "optixUtilAccumulateStackSizes(miss)");
            CheckOptix(optixUtilAccumulateStackSizes(hitgroupProgramGroup_, &stackSizes, pipeline_),
            ...
            CheckOptix(optixUtilComputeStackSizes(&stackSizes, 1, 0, 0, &directCallableStackTraversal,
                                                  &directCallableStackState, &continuationStack),
                       "optixUtilComputeStackSizes");
            CheckOptix(optixPipelineSetStackSize(pipeline_, directCallableStackTraversal, directCallableStackState,
                                                 continuationStack, 2), "optixPipelineSetStackSize");
        }
        ...
        void CreateCudaModule(const std::string &ptx) {
            ...
            CheckCudaDriver(cuModuleGetFunction(&toneMapKernel_, cudaModule_, "ToneMapToSurface"),
                            "cuModuleGetFunction(ToneMapToSurface)");
        }

        void CreateSbt() {
            RayGenRecord raygenRecord{};
            CheckOptix(optixSbtRecordPackHeader(raygenProgramGroup_, &raygenRecord),
                       "optixSbtRecordPackHeader(raygen)");
            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&raygenRecordBuffer_), sizeof(raygenRecord)),
                      "cudaMalloc(raygenRecord)");
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(raygenRecordBuffer_), &raygenRecord, sizeof(raygenRecord),
            ...
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(missRecordBuffer_), &missRecord, sizeof(missRecord),
                                 cudaMemcpyHostToDevice), "cudaMemcpy(missRecord)");

            CheckCuda(cudaMalloc(reinterpret_cast<void **>(&launchParamsBuffer_), sizeof(LaunchParams)),
                      "cudaMalloc(launchParamsBuffer)");
    ...
    public:
        int UploadMesh(
            ...
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(mesh.vertexBuffer), vertices, vertexBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(meshVertices)");
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(mesh.normalBuffer), normals, normalBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(meshNormals)");
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(mesh.texCoordBuffer), texCoords, texCoordBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(meshTexCoords)");
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(mesh.indexBuffer), indices, indexBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(meshIndices)");

            mesh.vertexCount = vertexCount;
            mesh.triangleCount = triangleCount;

            constexpr uint32_t flags[] = {OPTIX_GEOMETRY_FLAG_NONE};
            ...
            CheckOptix(optixAccelComputeMemoryUsage(context_, &accelOptions, &buildInput, 1, &sizes),
                       "optixAccelComputeMemoryUsage(mesh)");
        ...
        void UpdateMeshVertices(
            ...
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(mesh.vertexBuffer), vertices, vertexBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(updateVertices)");
            ...
            OptixAccelBufferSizes sizes{};
            CheckOptix(optixAccelComputeMemoryUsage(context_, &accelOptions, &buildInput, 1, &sizes),
                       "optixAccelComputeMemoryUsage(meshUpdate)");
        ...
        void EnsureMeshHitRecord(int meshId) {
            ...
            CheckOptix(optixSbtRecordPackHeader(hitgroupProgramGroup_, &hitRecord),
                       "optixSbtRecordPackHeader(hitgroup-mesh)");
            ...
            constexpr auto recordSize = sizeof(HitGroupRecord);
            ...
            if (hitgroupRecordCapacity_ < requiredSize) {
                ...
                CheckCuda(cudaMalloc(reinterpret_cast<void **>(&newBuffer), requiredSize),
                          "cudaMalloc(hitgroupRecords)");
                if (hitgroupRecordBuffer_ != 0) {
                    CheckCuda(cudaMemcpy(reinterpret_cast<void *>(newBuffer),
                                         reinterpret_cast<void *>(hitgroupRecordBuffer_), hitgroupRecordCapacity_,
                                         cudaMemcpyDeviceToDevice), "cudaMemcpy(hitgroupRecords)");
        ...
        void RenderInstances(
            ...
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(materialBuffer_), materials.data(), materialBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(materials)");
            if (texturePixelBytes > 0) {
                CheckCuda(cudaMemcpy(reinterpret_cast<void *>(texturePixelBuffer_), texturePixels, texturePixelBytes,
                                     cudaMemcpyHostToDevice), "cudaMemcpy(texturePixels)");
            }
            if (textureMetadataBytes > 0) {
                CheckCuda(cudaMemcpy(reinterpret_cast<void *>(textureMetadataBuffer_), textureMetadata,
                                     textureMetadataBytes, cudaMemcpyHostToDevice), "cudaMemcpy(textureMetadata)");
            }

            localStats.uploadSceneMs = std::chrono::duration<double, std::milli>(
            ...
            for (int i = 0; i < instanceCount; ++i) {
                ...
                if (static_cast<size_t>(meshId) >= meshes_.size() || meshes_[static_cast<size_t>(meshId)].gasHandle ==
                    0) {
            ...
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(iasInstanceBuffer_), optixInstances.data(), instancesBytes,
                                 cudaMemcpyHostToDevice), "cudaMemcpy(instances)");
            ...
            CheckOptix(optixAccelComputeMemoryUsage(context_, &iasOptions, &iasInput, 1, &iasSizes),
            ...
            localStats.uploadSceneMs += std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - iasStart).count();
            ...
            CheckCuda(cudaMemcpy(reinterpret_cast<void *>(launchParamsBuffer_), &params, sizeof(params),
                                 cudaMemcpyHostToDevice), "cudaMemcpy(launchParams)");
            ...
            localStats.launchMs = std::chrono::duration<double, std::milli>(
            ...
            const bool shouldDenoise = settings.enableDenoiser != 0 &&
                                       (denoiserFrameIndex == 0u || (
                                            denoiserFrameIndex % static_cast<unsigned int>(denoiserInterval)) == 0u);
            ...
            localStats.denoiseMs = std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - denoiseStart).count();

            const auto toneMapStart = std::chrono::steady_clock::now();
            PresentToOutputTexture(presentationSource, settings);
            localStats.toneMapMs = std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - toneMapStart).count();
            localStats.readbackMs = 0.0;
            localStats.totalMs = std::chrono::duration<double, std::milli>(
    ...
    private:
        static std::string CompileOptixPtx() {
            nvrtcProgram program = nullptr;
            CheckNvrtc(nvrtcCreateProgram(&program, kOptixDeviceSource, "rayoptix_optix_kernels.cu", 0, nullptr,
                                          nullptr), "nvrtcCreateProgram");

            cudaDeviceProp properties{};
            CheckCuda(cudaGetDeviceProperties(&properties, 0), "cudaGetDeviceProperties");

            std::string architecture = "--gpu-architecture=compute_" + std::to_string(properties.major) +
        ...
        static std::string CompilePresentPtx();
        ...
        void Destroy() {
            ...
            if (stream_ != nullptr) {
                IgnoreCudaDriver(cuStreamDestroy(stream_));
                stream_ = nullptr;
            }
            if (context_ != nullptr) {
    ...
    std::string NativeRenderer::CompilePresentPtx() {
        nvrtcProgram program = nullptr;
        CheckNvrtc(nvrtcCreateProgram(&program, kPresentDeviceSource, "rayoptix_present_kernels.cu", 0, nullptr,
                                      nullptr), "nvrtcCreateProgram");

        cudaDeviceProp properties{};
        CheckCuda(cudaGetDeviceProperties(&properties, 0), "cudaGetDeviceProperties");

        std::string architecture = "--gpu-architecture=compute_" + std::to_string(properties.major) + std::to_string(
                                       properties.minor);
        std::string cudaInclude = std::string("--include-path=") + ROPTIX_CUDA_INCLUDE_DIR;
